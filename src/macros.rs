...
macro_rules! mut_collection_methods {
    () => {
...
macro_rules! mut_set_methods {
    ($t:ty) => {
        fn insert(&mut self, item: $t) -> bool { self.insert(item) }
    }
}

macro_rules! mut_set_lookup_methods {
    ($q:ty) => {
